...
def test_skip_when_context_well_within_limit() -> None:
    ...
    assert len(result) == 1
    r0 = cast(ToolMessage, result[0])
    assert r0.tool_call_id == "call_1"
...
def test_prunes_largest_tool_first() -> None:
    ...
    result = prune(msgs, curr_context_usage=100_000, context_size=128_000)
    r0 = cast(ToolMessage, result[0])
    assert "[Old tool result content cleared" in r0.content
    r1 = cast(ToolMessage, result[1])
    assert r1.content == "short"
...
def test_preserves_non_tool_messages() -> None:
    ...
    r3 = cast(ToolMessage, result[3])
    assert "[Old tool result content cleared" in r3.content
...
def test_small_tools_below_min_savings_skipped() -> None:
    ...
    msgs: list[Message] = [_tool(_BIG, tool_call_id="call_0"), small_tool, _assistant()]
    result = prune(msgs, curr_context_usage=100_000, context_size=128_000)
...
def test_multiple_large_tools_all_pruned() -> None:
    ...
    assert "[Old tool result content cleared" in r0.content
    r1 = cast(ToolMessage, result[1])
    assert "[Old tool result content cleared" in r1.content


def test_preserves_tool_call_id_after_prune() -> None:
    """Tool call IDs are preserved in pruned messages."""
    msgs: list[Message] = [_tool("x" * PRUNE_PROTECT * 2, tool_call_id="call_xyz")]
    result = prune(msgs, curr_context_usage=100_000, context_size=128_000)
...
def test_prune_minimum_respected() -> None:
    ...
    many_msgs: list[Message] = [_tool(_MODERATE, tool_call_id=f"call_{i}") for i in range(10)]
    ...
    pruned_count = sum(
        1
        for m in result
        if isinstance(m, ToolMessage) and "[Old tool result content cleared" in m.content
    )
