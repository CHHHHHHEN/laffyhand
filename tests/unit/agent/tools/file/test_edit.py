...
class TestEditTool(unittest.TestCase):
    ...
    def test_edit_single_occurrence(self):
        ...
        f.write_text("foo\nbar\nbaz")
        tool = EditTool()
        result = asyncio.run(tool.run({"file_path": str(f), "old_string": "bar", "new_string": "qux"}))
    ...
    def test_edit_string_not_found(self):
        ...
        f.write_text("hello")
        tool = EditTool()
        result = asyncio.run(tool.run({"file_path": str(f), "old_string": "zzz", "new_string": "xxx"}))
    ...
    def test_edit_multiple_matches_no_replace_all(self):
        ...
        f.write_text("foo\nfoo\nfoo")
        tool = EditTool()
        result = asyncio.run(tool.run({"file_path": str(f), "old_string": "foo", "new_string": "bar"}))
        self.assertIn("Edited", result)
    ...
    def test_edit_replace_all(self):
        ...
        tool = EditTool()
        result = asyncio.run(tool.run({
            "file_path": str(f), "old_string": "foo", "new_string": "bar", "replaceAll": True,
        }))
    ...
    def test_edit_old_string_empty_prepends(self):
        ...
        tool = EditTool()
        result = asyncio.run(tool.run({
            "file_path": str(f), "old_string": "", "new_string": "prefix",
        }))
        self.assertIn("Edited", result)
    ...
    def test_edit_blocked_path(self):
        ...
        tool = EditTool()
        result = asyncio.run(tool.run({
            "file_path": str(f), "old_string": "OLD=val", "new_string": "NEW=val",
        }))
    ...
    def test_edit_directory_rejected(self):
        tool = EditTool()
        result = asyncio.run(tool.run({
            "file_path": str(self.root), "old_string": "x", "new_string": "y",
        }))
    ...
    def test_edit_relative_path_resolved(self):
        ...
        try:
            ...
            tool = EditTool()
            result = asyncio.run(tool.run({
                "file_path": "target.txt", "old_string": "content", "new_string": "modified",
            }))
            self.assertIn("Edited", result)
