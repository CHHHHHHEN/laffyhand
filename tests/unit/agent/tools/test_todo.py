...
class TestTodowriteTool:
    ...
    def test_add_and_read(self, tool, session_id):
        asyncio.run(tool.run({"operation": "add", "content": "test task", "priority": "high", "session_id": session_id}))
        result = asyncio.run(tool.run({"operation": "read", "session_id": session_id}))
    ...
    def test_plan_empty_tasks_returns_error(self, tool, session_id):
        result = asyncio.run(tool.run({"operation": "plan", "tasks": [], "session_id": session_id}))
    ...
    def test_update_status(self, tool, session_id):
        asyncio.run(tool.run({"operation": "add", "content": "task", "session_id": session_id}))
        tasks = asyncio.run(tool.run({"operation": "read", "session_id": session_id}))
        task_id = tasks.split("\n")[0].split()[0]
        result = asyncio.run(tool.run({"operation": "update", "id": task_id, "status": "completed", "session_id": session_id}))
        assert "completed" in result

    def test_update_requires_id(self, tool, session_id):
        result = asyncio.run(tool.run({"operation": "update", "session_id": session_id}))
        assert "id is required" in result

    def test_update_nonexistent_returns_error(self, tool, session_id):
        result = asyncio.run(tool.run({"operation": "update", "id": "no-such", "status": "completed", "session_id": session_id}))
        assert "not found" in result

    def test_update_with_content_and_priority(self, tool, session_id):
        asyncio.run(tool.run({"operation": "add", "content": "old", "session_id": session_id}))
        tasks = asyncio.run(tool.run({"operation": "read", "session_id": session_id}))
        task_id = tasks.split("\n")[0].split()[0]
        result = asyncio.run(tool.run({
            "operation": "update", "id": task_id,
            "content": "new content", "priority": "high",
            "session_id": session_id,
        }))
        assert "high" in result
    ...
    def test_delete(self, tool, session_id):
        ...
        tasks = asyncio.run(tool.run({"operation": "read", "session_id": session_id}))
        task_id = tasks.split("\n")[0].split()[0]
        asyncio.run(tool.run({"operation": "delete", "id": task_id, "session_id": session_id}))
        result = asyncio.run(tool.run({"operation": "read", "session_id": session_id}))
        assert "No tasks" in result

    def test_delete_unknown(self, tool, session_id):
        result = asyncio.run(tool.run({"operation": "delete", "id": "999", "session_id": session_id}))
        assert "not found" in result

    def test_unknown_operation(self, tool, session_id):
        result = asyncio.run(tool.run({"operation": "unknown", "session_id": session_id}))
        assert "Unknown operation" in result

    def test_read_with_status_filter(self, tool, session_id):
        asyncio.run(tool.run({"operation": "add", "content": "task1", "session_id": session_id, "priority": "high"}))
        tasks = asyncio.run(tool.run({"operation": "read", "session_id": session_id}))
        task_id = tasks.split("\n")[0].split()[0]
        asyncio.run(tool.run({"operation": "update", "id": task_id, "status": "completed", "session_id": session_id}))
        result = asyncio.run(tool.run({"operation": "read", "session_id": session_id, "status": "completed"}))
        assert "task1" in result

    def test_dag_plan_blocked_status_appears_in_read(self, tool, session_id):
        asyncio.run(tool.run({"operation": "plan", "tasks": [
    ...
    def test_add_with_depends_on(self, tool, session_id):
        asyncio.run(tool.run({"operation": "add", "content": "parent", "session_id": session_id}))
        tasks = asyncio.run(tool.run({"operation": "read", "session_id": session_id}))
