...
class TestRequest:
    ...
    async def test_returns_result(self, client, transport):
        transport.recv = AsyncMock(return_value=Response(id=1, result={"ok": True}).json())
...
class TestRequestStream:
    ...
    async def test_yields_notifications(self, client, transport):
        transport.recv = AsyncMock(side_effect=[
            Notification(method="event", params={"type": "delta", "data": "hello"}).json(),
...
class TestInitialize:
    ...
    async def test_returns_server_info(self, client, transport):
        transport.recv = AsyncMock(return_value=Response(id=1, result={
...
class TestSessionCreate:
    ...
    async def test_returns_session_id(self, client, transport):
        transport.recv = AsyncMock(return_value=Response(id=1, result={"session_id": "sess-1"}).json())
        sid = await client.create_session(system_prompt="hello")
        assert sid == "sess-1"

    @pytest.mark.anyio
    async def test_passes_params(self, client, transport):
        transport.recv = AsyncMock(return_value=Response(id=1, result={"session_id": "s-1"}).json())
        await client.create_session(system_prompt="sp", title="t", cwd="/tmp", model="m")
        sent = json.loads(transport.send.call_args[0][0])
...
class TestSessionList:
    ...
    @pytest.mark.anyio
    async def test_passes_params(self, client, transport):
        transport.recv = AsyncMock(return_value=Response(id=1, result={"sessions": []}).json())
...
class TestSessionLoad:
    ...
    async def test_returns_session_info(self, client, transport):
        transport.recv = AsyncMock(return_value=Response(id=1, result={
            "session_id": "sess-1", "messages_count": 3, "turn_count": 1,
        }).json())
...
class TestSessionDelete:
    ...
    async def test_deletes(self, client, transport):
        transport.recv = AsyncMock(return_value=Response(id=1, result={"status": "deleted"}).json())
...
class TestSessionFork:
    ...
    async def test_returns_child_id(self, client, transport):
        transport.recv = AsyncMock(return_value=Response(id=1, result={"session_id": "child-1"}).json())
...
class TestSetSessionTitle:
    ...
    async def test_sets_title(self, client, transport):
        transport.recv = AsyncMock(return_value=Response(id=1, result={"status": "ok"}).json())
...
class TestGenerateSessionTitle:
    ...
    async def test_returns_title(self, client, transport):
        transport.recv = AsyncMock(return_value=Response(id=1, result={"title": "Generated"}).json())
        title = await client.generate_session_title()
        assert title == "Generated"

    @pytest.mark.anyio
    async def test_returns_none(self, client, transport):
        transport.recv = AsyncMock(return_value=Response(id=1, result={"title": None}).json())
        title = await client.generate_session_title()
...
class TestArchiveSession:
    ...
    async def test_archives(self, client, transport):
        transport.recv = AsyncMock(return_value=Response(id=1, result={"status": "archived"}).json())
...
class TestCancelChat:
    ...
    async def test_cancels(self, client, transport):
        transport.recv = AsyncMock(return_value=Response(id=1, result={"status": "cancelled"}).json())
