...
class TestHandleTodoList:
    ...
    async def test_returns_tasks_for_session(self, runtime, transport):
        ...
        result = await handle_todo_list(runtime, {"session_id": "sess-1"}, transport, 1, "c1")
    ...
    async def test_filters_by_status(self, runtime, transport):
        ...
        result = await handle_todo_list(runtime, {"session_id": "sess-1", "status": "completed"}, transport, 1, "c1")
        assert len(result["tasks"]) == 1
        assert result["tasks"][0]["status"] == "completed"
        runtime.todo_manager.get_tasks.assert_called_once_with("sess-1", status="completed")
    ...
    async def test_returned_fields_match_snake_case_mapping(self, runtime, transport):
        task = _make_task("t1", content="test task", status="completed", priority="high",
                          depends_on=["dep1"])
...
class TestHandleTodoUpdate:
    ...
    async def test_updates_task_status(self, runtime, transport):
        runtime.todo_manager.update_task.return_value = _make_task("t1", status="completed")
        result = await handle_todo_update(
            runtime, {"task_id": "t1", "status": "completed", "session_id": "sess-1"},
            transport, 1, "c1",
        )
    ...
    async def test_raises_for_missing_task_id(self, runtime, transport):
        with pytest.raises(ValueError, match="task_id is required"):
            await handle_todo_update(runtime, {"session_id": "sess-1"}, transport, 1, "c1")
    ...
    async def test_raises_for_nonexistent_task(self, runtime, transport):
        ...
        with pytest.raises(ValueError, match="not found"):
            await handle_todo_update(
                runtime, {"task_id": "no-such", "session_id": "sess-1"},
                transport, 1, "c1",
            )
    ...
    async def test_updates_priority_and_content(self, runtime, transport):
        runtime.todo_manager.update_task.return_value = _make_task(
            "t1", content="new content", priority="high",
        )
        result = await handle_todo_update(
            runtime, {"task_id": "t1", "content": "new content", "priority": "high", "session_id": "sess-1"},
            transport, 1, "c1",
        )
