...
BUILTIN_AGENTS: dict[str, AgentInfo] = {
    "build": AgentInfo(
        ...
        "5. Use the TodoWrite tool to track multi-step tasks, marking items as completed individually\n"
         ...
         "Optionally enrich tags with structured metadata: --exports (dict of exported class/function/constant names), "
         "--side_effects (import-time side effects), --depends_on (external dependencies). "
         "To hide tag annotations from glob/read output, pass show_tags=false to those tools. "
    ...
    "general": AgentInfo(
        ...
        "- Verify your work if possible (run tests, check output)\n"
         ...
         "- Use the 'tag' tool to annotate files you create or modify with a macro-level description, "
         "and enrich with --exports, --side_effects, --depends_on where relevant. "
    ...
    "explore": AgentInfo(
        ...
        "- For clear communication, avoid using emojis\n"
         "# Tagging files (important)\n"
         "The tag system stores persistent semantic annotations so you (and other agents) "
         "can understand files without re-reading them.\n\n"
         "When to tag:\n"
         "- After reading a file that has NO tag → add one\n"
         "- After reading a file whose tag is stale (⚠️ STALE) → update it\n"
         "- For directories you explore, consider tagging them with a summary of their contents\n\n"
         "What to include in a tag:\n"
         "1. **--message**: A macro-level description of the file's overall purpose (required). "
         "Describe what the file DOES as a whole, not just one function. "
         "Think: 'if someone reads this tag 2 weeks later, what must they know?'\n"
         "2. **--exports** (recommended): A JSON dict of exported symbols the file exposes, "
         "each mapped to its kind: class/function/constant/type/variable. "
         "E.g. {\"validate_token\": \"function\", \"AuthError\": \"class\"}\n"
         "3. **--side_effects** (if applicable): Describe any import-time side effects, "
         "e.g. 'registers CLI commands on import' or 'loads environment variables'\n"
         "4. **--depends_on** (recommended): List external modules or files this file depends on, "
         "e.g. [\"httpx\", \"pydantic\", \"loguru\"]\n\n"
         "Rules:\n"
         "- Each file can only have ONE tag. Use 'tag add' to create or completely overwrite; "
         "'tag update' to modify specific fields without losing others.\n"
         "- To view all tags under a directory: 'tag list --path <dir>'\n"
         "- To hide tag annotations from glob/read output (for clean results): "
         "pass show_tags=false to those tools.\n"
         "- If you are exploring a large codebase, batch-tag related files "
         "using 'tag batch --tags [...]' for efficiency.\n\n"
        "# Constraints\n"
